...
function suggestEmail(a) {
    var b = "";
    $.ajax({type: "GET", async: !1, url: ctx + "/corpResume/forwardEmails.json", dataType: "json"}).done(function (a) {
        b = a.content
    }), $("#" + a).autocomplete({
        source: function (a, c) {
...
function popQR() {
    $.ajax({url: ctx + "/mycenter/showQRCode", type: "GET"}).done(function (a) {
...
$(function () {
    $("#filter_btn").bind("click", function () {
        ...
        var a = $(this), b = a.parents("li").attr("data-id"), c = $("#resubmitToken").val();
        return confirm("确定要删除该简历吗？") ? ($.ajax({
            type: "POST",
            url: ctx + "/corpResume/delete.json",
            data: {deliverIds: b, resubmitToken: c},
            dataType: "json"
        }).done(function (a) {
        ...
        var a = new Array, b = $("#resubmitToken").val();
        return $(".resumeLists li").each(function () {
            ...
            type: "POST",
            url: ctx + "/corpResume/delete.json",
            data: {deliverIds: a, resubmitToken: b},
            dataType: "json"
        }).done(function (a) {
            $("#resubmitToken").val(a.resubmitToken), a.success ? top.location.href = top.location.href : alert(a.msg)
        }), void 0) : !1
        ...
        onkeyup: !1,
        focusCleanup: !0,
        rules: {content: {required: !0, maxlength: 200}},
        messages: {content: {required: "请输入你的邮件内容", maxlength: "请输入200字符以内的邮件内容"}},
        submitHandler: function (a) {
            var b = $('textarea[name="content"]', a).val(), c = $('input[name="deliverId"]', a).val(), d = $("#resubmitToken").val();
            $.ajax({
                url: ctx + "/corpResume/markRefuse.json",
                type: "POST",
                data: {content: b, deliverIds: c, resubmitToken: d},
                dataType: "json"
            }).done(function (a) {
        ...
        var a = $(this).attr("data-deliverId"), b = $("#resubmitToken").val();
        $.ajax({
            type: "POST",
        ...
        focusCleanup: !0,
        rules: {
            recipients: {required: !0, moreEmail: !0},
            title: {required: !0, maxlength: 200},
            content: {required: !0, maxlength: 500}
        },
        messages: {
            recipients: {required: "请输入收件人的邮件地址", moreEmail: "请输入有效的收件人的邮件地址，最多2个，并用;隔开"},
            title: {required: "请输入你的主题", maxlength: "请输入200字符以内的主题"},
            content: {required: "请输入你的正文", maxlength: "请输入500字符以内的正文"}
        },
        submitHandler: function (a) {
            var b = $('input[name="recipients"]', a).val(), c = $('input[name="title"]', a).val(), d = $('textarea[name="content"]', a).val(), e = $('input[name="resumeKey"]', a).val(), f = $('input[name="deliverId"]', a).val(), g = $('input[name="positionId"]', a).val(), h = $("#resubmitToken").val();
            $.ajax({
                type: "POST",
        ...
        var a = $(this).attr("data-email"), b = $(this).attr("data-positionId"), c = $(this).attr("data-name"), d = $(this).attr("data-deliverId");
        $.ajax({
            type: "POST",
            url: ctx + "/corpResume/preNotice.json",
            data: {email: a, positionId: b, name: c},
            dataType: "json"
        }).done(function (a) {
        ...
        focusCleanup: !0,
        rules: {
            subject: {required: !0, maxlength: 200},
            interTime: {required: !0},
            interAdd: {required: !0, maxlength: 200},
            linkMan: {required: !1, rangelength: [2, 16]},
            linkPhone: {required: !0, isTel: !0},
            content: {required: !1, maxlength: 500}
        },
        messages: {
            subject: {required: "请输入你的主题", maxlength: "请输入200字符以内的主题"},
            interTime: {required: "请输入你的面试时间"},
            interAdd: {required: "请输入你的面试地点", maxlength: "请输入200字符以内的面试地点"},
            linkPhone: {required: "请输入你的联系电话", isTel: "请输入正确的手机号或座机号，座机格式如010-62555255或010-6255255-分机号，多个电话用英文逗号隔开"},
            linkMan: {maxlength: "请输入2-32个字符的联系人"},
            content: {maxlength: "请输入500字符以内的补充内容"}
        },
        submitHandler: function (a) {
            var b = $('input[name="name"]', a).val(), c = $('input[name="email"]', a).val(), d = $('input[name="subject"]', a).val(), e = $('input[name="interTime"]', a).val(), f = $('input[name="interAdd"]', a).val(), g = $('input[name="linkMan"]', a).val(), h = $('input[name="linkPhone"]', a).val(), i = $('textarea[name="content"]', a).val(), j = $('input[name="positionName"]', a).val(), k = $('input[name="companyName"]', a).val(), l = $('input[name="deliverId"]', a).val(), m = $("#resubmitToken").val();
            $.ajax({
                type: "POST",
                data: {
                    name: b,
        ...
        var j, k, a = $('#noticeInterviewForm input[name="name"]').val(), b = $('#noticeInterviewForm input[name="email"]').val(), c = $('#noticeInterviewForm input[name="subject"]').val(), d = $('#noticeInterviewForm input[name="interTime"]').val(), e = $('#noticeInterviewForm input[name="interAdd"]').val(), f = $('#noticeInterviewForm input[name="linkMan"]').val(), g = $('#noticeInterviewForm input[name="linkPhone"]').val(), h = $('#noticeInterviewForm textarea[name="content"]').val();
        ...
        $.colorbox({inline: !0, href: $("#downloadOnlineResume"), title: "下载简历"})
    }), $("#weixinQR .btn_s,#weixinQR .qr_cancel").click(function () {
        var a = new Date;
        $.cookie("showQRCode", a.getDate(), {expired: 180}), $.colorbox.close(), parent.jQuery.colorbox.close()
        ...
        var a = $(this), b = a.attr("data-forwardCount"), c = a.attr("data-deliverId");
