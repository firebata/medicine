function checkCity(a) {
    var b;
    return $.ajax({type: "POST", async: !1, url: ctx + "/jobs/checkCity.json", data: {city: a}}).done(function (a) {
        b = a.success
    }), b
}
...
$(function () {
    function a() {
        ...
        $("#jobForm").valid() && ($("#department").val() == $("#department").attr("placeholder") && $("#department").val(""), ("" == $("#salaryMin").val() || $("#salaryMin").val() == $("#salaryMin").attr("placeholder")) && $("#salaryMin").val(""), ("" == $("#salaryMax").val() || $("#salaryMax").val() == $("#salaryMax").attr("placeholder")) && $("#salaryMax").val(""), $("#positionAddress").val() == $("#positionAddress").attr("placeholder") && $("#positionAddress").val(""), a = $("#positionType").val(), b = $("#positionName").val(), c = $("#department").val(), d = $('#jobForm input[name="jobNature"]:checked').val(), e = $("#salaryMin").val(), f = $("#salaryMax").val(), g = $("#workAddress").val(), h = $("#experience").val(), i = $("#education").val(), j = $("#positionAdvantage").val(), k = $("#positionDetail").val(), l = $("#positionAddress").val(), m = $("#lng").val(), n = $("#lat").val(), o = $("#receiveEmail").val(), p = $("#forwardEmail").val(), q = $('#jobForm input[name="id"]').val(), r = $('#jobForm input[name="companyId"]').val(), s = $("#resubmitToken").val(), $.ajax({
    ...
    $("#telForm").validate({
        ...
        submitHandler: function (a) {
            ...
            $(a).find(":submit").attr("disabled", !0), $.ajax({
                ...
                b.success ? $.colorbox.close() : $("#telError").text(b.msg).show(), $(a).find(":submit").attr("disabled", !1)
            })
        }
        ...
        $(this).parents("#box_experience").hide().siblings("#select_experience").val(b).css("color", "#333").removeClass("selectrFocus"), $(this).parents("#box_experience").hide().siblings("#experience").val(b), $("#jobForm").validate().element($("#experience"))
        ...
        invalidHandler: function (a, b) {
            if (b.numberOfInvalids()) {
                ...
                "positionType" == b.errorList[0].element.name ? c = 150 : "workYear" == b.errorList[0].element.name ? c = 650 : "education" == b.errorList[0].element.name && (c = 700), $("html, body").animate({scrollTop: c}, 400)
            }
        }
    }), jQuery.validator.addMethod("cityAvailable", function (a, b) {
        return this.optional(b) || checkCity(a)
    }, "请输入有效的公司所在城市，如：广州"), jQuery.validator.addMethod("hasEmail", function (a) {
    ...
    }, "请输入20-2000字的职位描述"), $.validator.classRuleSettings.textInMce = {textInMce: !0}, $("#workAddress").focus(function () {
        $("#beError").hide()
    }), jQuery.validator.addMethod("forwardEmailFormat", function (a) {
        var d = $.trim(a).indexOf("@"), e = $.trim(a).substring(d, $.trim(a).length), f = $.trim($("#receiveEmailVal").text()), g = f.indexOf("@"), h = f.substring(g, f.length);
        return "" != $.trim(a) && e != h ? !1 : !0
    }, "请输入与当前接收简历邮箱后缀一致的邮箱地址"), jQuery.validator.addMethod("forwardSame", function (a) {
        var d = $.trim(a), e = $.trim($("#receiveEmailVal").text());
        return "" != d && d == e ? !1 : !0
    }, "请输入与当前接收简历邮箱不同的邮箱地址"), $("#jobPreview").click(function () {
