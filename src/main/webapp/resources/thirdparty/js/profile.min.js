function scoreChange(a) {
    function e() {
        ...
        new Chart(b).Doughnut(c, d), $("#resumeScore .scoreVal span").text(a).parent().fadeIn(200), 100 == a ? $("#resumeScore .which span").html('<a href="' + ctx + '/jobs/list_所有职位">马上去投递</a>') : $("#resumeScore .which span").html("<a>马上去完善</a>")
    }

    var b = 360 * parseInt(a) / 100, c = [{value: b, color: "#009a6d"}, {
        value: 360 - b,
        color: "#ffffff"
...
function changeAllIds(a) {
    ...
    $("#resume_id").val(b.resume_id.n), c = b.expId, d = b.eduId, e = b.projectId, f = b.showId, $("#workExperience .wlist li").each(function () {
        var b, a = $(this).attr("data-id");
        for (b = 0; b < c.length; b++)a == c[b].o && $(this).attr("data-id", c[b].n)
    }), $("#projectExperience .plist li").each(function () {
        var b, a = $(this).attr("data-id");
        for (b = 0; b < e.length; b++)a == e[b].o && $(this).attr("data-id", e[b].n)
    }), $("#educationalBackground .elist li").each(function () {
        var b, a = $(this).attr("data-id");
        for (b = 0; b < d.length; b++)a == d[b].o && $(this).attr("data-id", d[b].n)
    }), $("#worksShow .slist li").each(function () {
        var b, a = $(this).attr("data-id");
        for (b = 0; b < f.length; b++)a == f[b].o && $(this).attr("data-id", f[b].n)
    })
}
function openYear(a) {
    $(".profile_select_normal").removeClass("select_focus"), $(".boxUpDown").hide(), $(".select_" + a).addClass("select_focus"), $(".box_" + a).show()
}
function selectYear(a, b) {
    $(".select_" + a).val(b).css("color", "#333").removeClass("select_focus"), $("." + a).val(b), $(".box_" + a).hide()
}/*
...
function suggestPosition(a, b) {
    var c = "";
    $.ajax({type: "POST", async: !1, url: ctx + "/jobs/listLs", data: {pl: a}, dataType: "json"}).done(function (a) {
...
function click_scroll(a) {
    ...
    switch (a) {
        case"expectJob":
            $("#expectJob .expectAdd").trigger("click");
            break;
        case"workExperience":
            $("#workExperience .experienceAdd").trigger("click");
            break;
        case"projectExperience":
            $("#projectExperience .projectAdd").trigger("click");
            break;
        case"educationalBackground":
            $("#educationalBackground .educationalAdd").trigger("click");
            break;
        case"selfDescription":
            $("#selfDescription .descriptionAdd").trigger("click");
            break;
        case"worksShow":
            $("#worksShow .workAdd").trigger("click");
            break;
        default:
            return
    }
...
function check_img(a, b, c) {
    ...
    this.AllowExt = ".jpg,.gif,.jpeg,.png,.pjpeg", this.FileExt = a.val().substr(a.val().lastIndexOf(".")).toLowerCase(), 0 != this.AllowExt && -1 == this.AllowExt.indexOf(this.FileExt) ? errorTips("只支持jpg、gif、png、jpeg格式，请重新上传", "上传头像") : ($("#" + c + "_error").text("").hide(), $.ajaxFileUpload({
        ...
        success: function (a) {
            if (a.success) {
                var b = a.content;
                $("#portraitShow img").attr("src", ctx + "/" + b), $("#basicInfo .basicShow img").attr("src", ctx + "/" + b), $("#portraitNo").hide(), $("#portraitShow").show()
...
function file_check(obj, action_url, id) {
    var userId;
    $("#loadingImg").css("visibility", "visible"), obj = $("#" + id), userId = $("#userid").val(), this.AllowExt = ".doc,.docx,.pdf,.ppt,.pptx,.txt,.wps", this.FileExt = obj.val().substr(obj.val().lastIndexOf(".")).toLowerCase(), 0 != this.AllowExt && -1 == this.AllowExt.indexOf(this.FileExt) ? (errorTips("只支持word、pdf、ppt、txt、wps格式文件，请重新上传"), $("#loadingImg").css("visibility", "hidden")) : $.ajaxFileUpload({
        type: "POST",
        url: action_url,
        secureuri: !1,
        fileElementId: id,
...
!function (a, b) {
    ...
    var n, f = "backgroundColor borderBottomColor borderLeftColor borderRightColor borderTopColor color columnRuleColor outlineColor textDecorationColor textEmphasisColor", g = /^([\-+])=\s*(\d+\.?\d*)/, h = [{
        ...
        $(".projectForm").each(function () {
            $(this).validate({
                ...
                submitHandler: function (a) {
                    var b = $('input[name="projectName"]', a).val(), c = $('input[name="thePost"]', a).val(), e = $('input[name="projectYearStart"]', a).val(), f = $('input[name="projectMonthStart"]', a).val(), g = $('input[name="projectYearEnd"]', a).val(), h = $('input[name="projectMonthEnd"]', a).val(), i = "项目描述" != $('textarea[name="projectDescription"]', a).val() ? $('textarea[name="projectDescription"]', a).val() : "", j = $(".projectId", a).val(), l = $("#resume_id").val(), m = $("#resubmitToken").val();
                    $(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
                        url: ctx + "/projectExperience/save.json",
                        type: "POST",
                        data: {
                            projectName: b,
                            position_name: c,
    ...
    function i() {
        $(".workForm").each(function () {
            $(this).validate({
                ...
                submitHandler: function (a) {
                    var b = "请输入作品链接" != $('input[name="workLink"]', a).val() ? $('input[name="workLink"]', a).val() : "", c = "请输入说明文字" != $('textarea[name="workDescription"]', a).val() ? $('textarea[name="workDescription"]', a).val() : "", d = $(".showId", a).val(), e = $("#resume_id").val(), f = $("#resubmitToken").val();
                    $(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
    ...
    window.onbeforeunload = function () {
        ...
        return d > e ? !1 : !0
    }, "开始时间需小于结束时间"), $("#resume_name .nameShow .rename").bind("click", function () {
        ...
        submitHandler: function (a) {
            var b = $('input[name="resumeName"]', a).val(), c = $("#resume_id").val();
            $(a).find(":submit").attr("disabled", !0), $.ajax({
                url: ctx + "/resume/renameResume.json",
                type: "POST",
                data: {resumeName: b, id: c}
            }).done(function (b) {
                if (b.success) {
                    var c = "";
                    c = b.content.resumeName.length > 18 ? b.content.resumeName.substring(0, 15) + "..." : b.content.resumeName, $("#resume_name .nameShow h1").attr("title", b.content.resumeName).text(c).parent().removeClass("dn"), $("#resume_name form").addClass("dn"), $("#lastChangedTime span").text(b.content.refreshTime), k($("#resume_name"))
                } else alert(b.msg);
                $(a).find(":submit").attr("disabled", !1)
            })
        }
    }), $("#basicInfo .c_edit").bind("click", function () {
        var a = $("#nameVal").val(), b = $("#genderVal").val(), c = $("#topDegreeVal").val(), d = $("#workyearVal").val(), e = $("#currentStateVal").val(), f = $("#emailVal").val(), g = $("#telVal").val();
        $("#name").val(a), $('#basicInfo .profile_radio li input[value="' + b + '"]').attr("checked", !0).parent("li").addClass("current").siblings("li").removeClass("current"), $("#topDegree").val(c), $("#select_topDegree").val(c).css("color", "#333"), $("#workyear").val(d), $("#select_workyear").val(d).css("color", "#333"), "" != e && ($("#currentState").val(e), $("#select_currentState").val(e).css("color", "#333")), $("#email").val(f), $("#tel").val(g), $("#portraitShow img").attr("src", $("#basicInfo .basicShow img").attr("src")), $("#portraitNo").hide(), $("#portraitShow").show(), $(this).addClass("dn"), $("#basicInfo .basicShow").addClass("dn"), $("#basicInfo .basicEdit").removeClass("dn"), j($("#basicInfo"))
    }), $("#select_topDegree").bind("click", function (a) {
        a.stopPropagation(), $(this).addClass("select_focus"), $("#select_workyear").removeClass("select_focus"), $("#select_currentState").removeClass("select_focus"), $("#box_topDegree").show(), $("#box_workyear").hide(), $("#box_currentState").hide()
    }), $("#box_topDegree").on("click", "ul li", function (a) {
        a.stopPropagation();
        var b = $(this).text();
        $("#select_topDegree").val(b).css("color", "#333").removeClass("select_focus"), $("#academic_name").val(b), $("#box_topDegree").hide(), $("#profileForm").validate().element($("#academic_name"))
    }), $("#select_workyear").bind("click", function (a) {
        a.stopPropagation(), $(this).addClass("select_focus"), $("#select_topDegree").removeClass("select_focus"), $("#select_currentState").removeClass("select_focus"), $("#box_workyear").show(), $("#box_topDegree").hide(), $("#box_currentState").hide()
    }), $("#box_workyear").on("click", "ul li", function (a) {
        a.stopPropagation();
        var b = $(this).text();
        $("#select_workyear").val(b).css("color", "#333").removeClass("select_focus"), $("#seniority_name").val(b), $("#box_workyear").hide(), $("#profileForm").validate().element($("#seniority_name"))
    }), $("#select_currentState").bind("click", function (a) {
        a.stopPropagation(), $(this).addClass("select_focus"), $("#select_topDegree").removeClass("select_focus"), $("#select_workyear").removeClass("select_focus"), $("#box_workyear").hide(), $("#box_topDegree").hide(), $("#box_currentState").show()
    }), $("#box_currentState").on("click", "ul li", function (a) {
        a.stopPropagation();
        var b = $(this).text();
        $("#select_currentState").val(b).css("color", "#333").removeClass("select_focus"), $("#state_name").val(b), $("#box_currentState").hide(), $("#profileForm").validate().element($("#state_name"))
    }), $("#profileForm").validate({
        rules: {
            name: {required: !0, specialchar: !0, checkNum: !0, rangelength: [2, 20]},
            sex_name: {required: !1},
            academic_name: {required: !0},
            seniority_name: {required: !0},
            state_name: {required: !1},
            mail_box: {required: !0, email: !0, maxlength: 80},
            phone: {required: !0, isMobile: !0, maxlength: 30}
        },
        messages: {
            name: {
                required: "请输入你的真实姓名",
            ...
            academic_name: {required: "请选择最高学历"},
            seniority_name: {required: "请选择工作年限"},
            mail_box: {
                required: "请输入接收面试通知的常用邮箱",
                email: "请输入有效的常用邮箱，如：vivi@zzlinks.cn",
                maxlength: "请输入有效的常用邮箱，如：vivi@zzlinks.cn"
            },
            phone: {required: "请输入手机号码", isMobile: "请输入有效的手机号码", maxlength: "请输入有效的手机号码"}
        },
        errorPlacement: function (a, b) {
            "hidden" == b.attr("type") ? a.appendTo($(b).parent()) : "radio" == b.attr("type") ? a.insertAfter($(b).parents("ul")) : a.insertAfter(b)
        },
        submitHandler: function (a) {
            var b = $('input[name="name"]', a).val(), c = $('input[name="sex_name"]:checked', a).val(),
                d = $('input[name="topDegree"]', a).val(), e = $('input[name="workyear"]', a).val(),
                f = $('input[name="currentState"]', a).val(), g = $('input[name="email"]', a).val(),
                h = $('input[name="tel"]', a).val(), i = $("#pageType").val(), j = $("#resume_id").val(), l = $("#resubmitToken").val();
            $(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
                url: employeeUrl + $("#employee_id").val(),
                type: "PUT",
                data: {
                    name: b,
                    sex: c,
        ...
        a.stopPropagation();
        var b = $(this).text();
        $("#select_expectCity").val(b).css("color", "#333").removeClass("select_focus"), $("#area_name").val(b), $("#box_expectCity").hide(), $("#expectForm").validate().element($("#area_name")),$("#expectForm").validate().element($("#position_name"))
        ...
        a.stopPropagation();
        var b = $(this).text();
        $("#select_expectSalary").val(b).css("color", "#333").removeClass("select_focus"), $("#salary_name").val(b), $("#box_expectSalary").hide(), $("#profileForm").validate().element($("#salary_name"))
    }), b = {
        obj: $("#expectJob"),
        ...
        Add: function () {
            this.obj.find(".expectAdd").addClass("dn"), this.obj.find(".expectShow").addClass("dn"),
                this.obj.find(".c_edit").addClass("dn"), this.obj.find(".expectEdit").removeClass("dn"), j(this.obj)
        },
        Edit: function () {
            /*$("#expectJob .expectList").addClass("dn");*/
            this.typeVal = $("#typeVal").val();
            this.expectCityVal = $("#expectCityVal").val();
            this.expectPositionVal = $("#expectPositionVal").val();
            this.expectSalaryVal = $("#expectSalaryVal").val();
            "" != this.typeVal ?
                 $('.profile_radio li input[value="' + this.typeVal + '"]').attr("checked", !0).parent("li").addClass("current").siblings("li").removeClass("current")
                : $(".profile_radio li input").attr("checked", !1).parent("li").removeClass("current");
                "" == this.expectCityVal ? ($("#expectCity").val(""), $("#select_expectCity").val("期望城市，如：广州").css("color", "#999")) : ($("#area_name").val(this.expectCityVal), $("#select_expectCity").val(this.expectCityVal).css("color", "#333")), "" != this.typeVal ? $('.profile_radio li input[value="' + this.typeVal + '"]').attr("checked", !0).parent("li").addClass("current").siblings("li").removeClass("current") : $(".profile_radio li input").attr("checked", !1).parent("li").removeClass("current"), $("#expectPosition").val(this.expectPositionVal), "" == this.expectSalaryVal ? ($("#expectSalary").val(""), $("#select_expectSalary").val("期望月薪").css("color", "#999")) : ($("#salary_name").val(this.expectSalaryVal), $("#select_expectSalary").val(this.expectSalaryVal).css("color", "#333")), placeholderFn(), this.obj.find(".expectAdd").addClass("dn"), this.obj.find(".expectShow").addClass("dn"), this.obj.find(".c_edit").addClass("dn"), this.obj.find(".expectEdit").removeClass("dn"), j(this.obj)
        },
        Cancel: function () {


            $("#expectCityVal").val() || $("#typeVal").val() || $("#expectPositionVal").val() || $("#expectSalaryVal").val() ?
                this.obj.find(".expectEdit").addClass("dn").
                siblings(".expectShow").removeClass("dn").siblings(".c_edit").removeClass("dn").siblings(".expectAdd").addClass("dn")
                :
                this.obj.find(".expectEdit").addClass("dn").
                siblings(".expectShow").addClass("dn").siblings(".c_edit").addClass("dn").siblings(".expectAdd").removeClass("dn"), k(this.obj)
        }
        ...
        rules: {
            area_name: {required: !1},
            type_name: {required: !1},
            position_name: {required: !1, specialchar: !0, checkNum: !0, minlength: 2, maxlength: 100},
            salary_name: {required: !1}
        },
        messages: {
            position_name: {
        ...
        submitHandler: function (a) {
            var c = $('input[name="expectCity"]', a).val(), d = $('input[name="type"]:checked', a).val(), e = "期望职位，如：产品经理" != $('input[name="expectPosition"]', a).val() ? $('input[name="expectPosition"]', a).val() : "", f = $('input[name="expectSalary"]', a).val(), g = $("#resume_id").val(), h = $("#resubmitToken").val();
            $(a).find(":submit").val("保存中...").attr("disabled", !0)/*, $.ajax({
                url: ctx + "/resume/expect.json",
                type: "POST",
                data: {city: c, positionType: d, position_name: e, salarys: f, id: g, resubmitToken: h},
                dataType: "json"
            }).done(function (g) {
                if ($("#resubmitToken").val(g.resubmitToken), g.success) {
                    if (c || d || e || f) {
                        var h = g.content.expectJob;
                        b.obj.find(".expectShow").children("span").html(h).parent().removeClass("dn"), b.obj.find(".c_edit").removeClass("dn"), b.obj.find(".expectEdit").addClass("dn"), b.obj.find("#expectJobVal").val(g.content.resume.expectJob), b.obj.find("#expectCityVal").val(g.content.resume.city), b.obj.find("#typeVal").val(g.content.resume.positionType), b.obj.find("#expectPositionVal").val(g.content.resume.position_name), b.obj.find("#expectSalaryVal").val(g.content.resume.salarys), b.expectJobVal = g.content.resume.expectJob, b.expectCityVal = g.content.resume.city, b.typeVal = g.content.resume.positionType, b.expectPositionVal = g.content.resume.position_name, b.expectSalaryVal = g.content.resume.salarys, $("#lastChangedTime span").text(g.content.refreshTime), $("#resumeScore .which div").text(g.content.infoCompleteStatus.msg), $("#resumeScore .which span").attr("rel", g.content.infoCompleteStatus.nextStage), scoreChange(g.content.infoCompleteStatus.score), k(b.obj)
        ...
        Add: function () {
            ...
            this.obj.children(".experienceEdit").find(".company_name").val(""),
                this.obj.children(".experienceEdit").find(".position_name").val(""),
                ...
                placeholderFn(), this.obj.children(".experienceAdd").addClass("dn"), this.obj.children(".experienceShow").addClass("dn"),
                this.obj.children(".experienceEdit").removeClass("dn"),
                $(".experienceShow").removeClass("dn"),
                $(".experienceForm.borderBtm").removeClass("dn"),
                tinyMCE.get('experienceText').setContent(""),
                j(this.obj)
        },
        AddMore: function () {
            this.obj.children(".experienceShow").find(".company_name").val(""), this.obj.children(".experienceShow").find(".position_name").val(""), this.obj.children(".experienceShow").find(".companyYearStart").val(""), this.obj.children(".experienceShow").find(".select_companyYearStart").css("color", "#999").val("开始年份"), this.obj.children(".experienceShow").find(".companyMonthStart").val(""), this.obj.children(".experienceShow").find(".select_companyMonthStart").css("color", "#999").val("开始月份"), this.obj.children(".experienceShow").find(".companyYearEnd").val(""), this.obj.children(".experienceShow").find(".select_companyYearEnd").css("color", "#999").val("结束年份"), this.obj.children(".experienceShow").find(".companyMonthEnd").val(""), this.obj.children(".experienceShow").find(".select_companyMonthEnd").css("color", "#999").val("结束月份"), this.obj.children(".experienceShow").find(".expId").val(""),
                placeholderFn(), this.obj.children(".experienceShow").children(".experienceForm").show(),
                $("input[name='company_position_name']").val(""),
                tinyMCE.get('experienceText').setContent(""),
                j(this.obj)
        },
        Edit: function (a) {
            //alert(b = a.parent("li").attr("data-id"));
            console.info(a.parent("li").find(".descriptionShow").html());
            var h, i, b = a.parent("li").attr("data-id"), c = a.siblings("span").attr("data-startYear"),
                d = a.siblings("span").attr("data-endYear"), e = a.siblings("span").attr("data-startMonth"),
                f = a.siblings("span").attr("data-endMonth");
            a.parent("li").find("img").attr("src"), h = a.parent("li").find("h3").text(), i = a.parent("li").find("h4").text(),
                this.obj.children(".experienceShow").find(".company_name").val(i),
                this.obj.children(".experienceShow").find(".position_name").val(h),
                ...
                this.obj.children(".experienceShow").find(".expId").val(b), this.obj.children(".experienceShow").children(".experienceForm").show(),
                tinyMCE.get('experienceText').setContent(a.parent("li").find(".descriptionShow").html()),
            "至今" == $.trim(f) && this.obj.children(".experienceShow").find(".select_companyMonthEnd").unbind("click"),
                this.companyNameVal = i, this.positionNameVal = h, this.companyYearStartVal = c, this.companyMonthStartVal = e, this.companyYearEndVal = d,
                this.companyMonthEndVal = f, this.obj.children(".c_add").addClass("dn"),
                
                j(this.obj)
        },
        Del: function (a) {
           /* var b = a.parent("li").attr("data-id");
            $.ajax({url: ctx + "/workExperience/delExp.json", type: "POST", data: {id: b}}).done(function (b) {
                b.success ? (a.parent("li").remove(), "" == b.content.workExperiences || null == b.content.workExperiences ? 
                    (c.obj.children(".experienceShow").addClass("dn"), c.obj.children(".experienceEdit").addClass("dn"), 
                        c.obj.children(".experienceAdd").removeClass("dn"), c.obj.children(".c_add").addClass("dn")) :
                    c.obj.children(".experienceShow").children(".experienceForm").hide(), $("#lastChangedTime span").text(b.content.refreshTime),
                    ...
                    scoreChange(b.content.infoCompleteStatus.score), k(c.obj), b.content.isNew && changeAllIds(b.content.jsonIds)) : alert(b.msg)
            })*/
            var b = a.parent("li").attr("data-id");
            $.delCnfwsy(empResCompanyUrl+ b , {}, function (data) {
/*
                $(".experienceShow").addClass("dn");
                $(".experienceEdit").addClass("dn");
                $(".experienceShow .experienceForm").hide();
                $(".c_add").addClass("dn")*/
                /*$(".experienceShow .wlist li").length?*/
                $(".experienceShow .wlist li").eq(-1).remove();
                var len = $(".experienceShow .wlist li");
                len.eq(-1).remove();
                if(len.length == 1){
                    $("#workExperience .c_add").addClass("dn");
                    $(".experienceAdd.pAdd").removeClass("dn");
                }
        ...
        AddCancel: function () {
            this.obj.find(".experienceAdd").removeClass("dn"), this.obj.find(".experienceShow").addClass("dn"), 
                this.obj.find(".experienceEdit").addClass("dn"), k(this.obj)
        },
        AddMoreCancel: function () {
            //alert($("#expId").val());
            if($(".experienceShow .wlist li").length ) {
    ...
    }, c.obj.find(".experienceAdd").bind("click", function () {
        c.Add()
    }), c.obj.find(".c_add").bind("click", function () {
        c.AddMore(), $(this).addClass("dn")
    }), c.obj.on("click", ".sm_edit", function () {
        c.Edit($(this))
    }), c.obj.on("click", ".sm_del", function () {
    ...
    }), c.obj.children(".experienceEdit").find(".btn_profile_cancel").bind("click", function () {
        c.AddCancel()
    }), c.obj.children(".experienceShow").find(".btn_profile_cancel").bind("click", function () {
        c.AddMoreCancel()/*, c.obj.find(".c_add").removeClass("dn")*/
    }), /*c.obj.delegate(".company_name", "keyup", function () {
        var a = $.trim($(this).val());
        "" != a && suggestCompany(a, ".company_name")
    }), c.obj.delegate(".position_name", "keyup", function () {
        var a = $.trim($(this).val());
        "" != a && suggestPosition(a, ".position_name")
        ...
        $(this).validate({
            ...
            rules: {
                company_name: {required: !0, maxlenStr: 100},
                position_name: {required: !0, checkNum: !0, maxlenStr: 100},
                company_start_year: {required: !0, checkWorkYear: !0},
                company_start_month: {required: !0, checkWorkYear: !0},
                company_end_year: {required: !0, checkWorkYear: !0},
                company_end_month: {
                    required: function () {
            ...
            messages: {
                company_name: {required: "请输入有效的公司名称，如：招徕网", maxlenStr: "请输入有效的公司名称，如：招徕网"},
                position_name: {
                    required: "请输入有效的职位名称，如：产品经理",
                    checkNum: "请输入有效的职位名称，如：产品经理",
                    maxlenStr: "请输入有效的职位名称，如：产品经理"
                },
                company_start_year: {required: "请选择开始时间", checkWorkYear: "开始时间需早于结束时间"},
                company_start_month: {required: "请选择开始时间", checkWorkYear: "开始时间需早于结束时间"},
                company_end_year: {required: "请选择结束时间", checkWorkYear: "结束时间需晚于开始时间"},
                company_end_month: {required: "请选择结束时间", checkWorkYear: "结束时间需晚于开始时间"}
            },
            errorPlacement: function (a, b) {
            ...
            submitHandler: function (a) {
                var b = $('input[name="company_name"]', a).val(), d = $('input[name="position_name"]', a).val(), e = $('input[name="company_start_year"]', a).val(),
                    f = $('input[name="company_start_month"]', a).val(), g = $('input[name="company_end_year"]', a).val(),
                    h = $('input[name="company_end_month"]', a).val(), i = $(".expId", a).val() /!*j = $("#resume_id").val(), l = $("#resubmitToken").val()*!/;
                $(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
                    url: ctx + "/workExperience/save.json",
                    type: "POST",
                    data: {
                        company_name: b,
                        position_name: d,
                    ...
                    if ($("#resubmitToken").val(b.resubmitToken), b.success) {
                        for (d = b.content.workExperiences, e = "", f = 0; f < d.length; f++)e += 0 == f % 2 ? '<li data-id="' + d[f].id + '" class="clear">' 
                        : '<li data-id="' + d[f].id + '">', e += '<i class="sm_del dn"></i><i class="sm_edit dn"></i><span class="c9" data-startYear="' 
                        + d[f].startYear + '" data-endYear="' + d[f].endYear + '" data-startMonth = "' + d[f].startMonth + '" data-endMonth = "' +
                         d[f].endMonth + '">' + d[f].startDate + "-" + d[f].endDate + "</span>" + "<div>" + '<img src="' + ctx + "/" + d[f].companyLogo +
                          '" width="56" height="56" alt="' + d[f].company_name + '" />' + "<h3>" + d[f].position_name + "</h3>" + "<h4>" + d[f].company_name + 
                          "</h4>" + "</div>" + "</li>";
                        ...
                        c.obj.children(".experienceEdit").addClass("dn"), $("#lastChangedTime span").text(b.content.refreshTime), 
                        $("#resumeScore .which div").text(b.content.infoCompleteStatus.msg), 
                        $("#resumeScore .which span").attr("rel", b.content.infoCompleteStatus.nextStage), 
                        scoreChange(b.content.infoCompleteStatus.score), k(c.obj), b.content.isNew && changeAllIds(b.content.jsonIds)
                    } else alert(b.msg);
                    $(a).find(":submit").val("保 存").attr("disabled", !1)
                })
            }*/
        ...
        obj: $("#projectExperience"), resetForm: function () {
            ...
            $.ajax({url: ctx + "/projectExperience/delProject.json", type: "POST", data: {id: b}}).done(function (b) {
    ...
    }, d.obj.find(".plist li:last-child").addClass("noborder"), d.obj.find(".projectAdd").bind("click", function () {
        d.Add()
    }), d.obj.find(".c_add").bind("click", function () {
        d.AddMore(), $(this).addClass("dn")
    }), d.obj.on("click", ".sm_edit", function () {
        d.Edit($(this))
    }), d.obj.on("click", ".sm_del", function () {
    ...
    }), d.obj.on("keyup", ".s_textarea", function () {
        var a = $(this);
        $.trim(a.val()).length > 500 ? a.val($.trim(a.val()).substring(0, 500)) : a.next(".word_count").children("span").text(500 - $.trim(a.val()).length)
    }), d.obj.children(".projectEdit").find(".btn_profile_cancel").bind("click", function () {
        d.AddCancel()
    }), d.obj.children(".projectShow").on("click", ".btn_profile_cancel", function () {
        d.AddMoreCancel($(this)), d.obj.find(".c_add").removeClass("dn")
    }), e(), $(".select_schoolYearStart").bind("click", function (a) {
        ...
        Del: function (a) {
            ...
            $.delCnfwsy(empResEduUrl+ b , {}, function (data) {
                /*
                 ...
                 $(".c_add").addClass("dn")*/
                /*$(".experienceShow .wlist li").length?*/
                var len = $(".educationalShow .elist li");
                len.eq(-1).remove();
                if(len.length == 1){
                    $("#eduAddMoreCancel").trigger("click");
                }
    ...
    }, f.obj.find(".educationalAdd").bind("click", function () {
        f.Add()
    }), f.obj.find(".c_add").bind("click", function () {
        f.AddMore(), $(this).addClass("dn")
    }), f.obj.on("click", ".sm_edit", function () {
        f.Edit($(this))
    }), f.obj.on("click", ".sm_del", function () {
    ...
    }), f.obj.children(".educationalEdit").find(".btn_profile_cancel").bind("click", function () {
        f.AddCancel()
    }), f.obj.children(".educationalShow").find(".btn_profile_cancel").bind("click", function () {
        f.AddMoreCancel(), f.obj.find(".c_add").removeClass("dn")
        ...
        $(this).validate({
            ...
            messages: {
                schoolName: {required: "请输入学校名称", checkNum: "请输入有效的学校名称", maxlenStr: "请输入有效的学校名称"},
                ...
                schoolYearStart: {required: "请选择开始年份", checkSchoolYear: "开始年份需小于结束年份"},
                schoolYearEnd: {required: "请选择结束年份", checkSchoolYear: "结束年份不能早于开始年份"}
            },
            errorPlacement: function (a, b) {
            ...
            submitHandler: function (a) {
                ///alert($("#resume_id").val());
                var b = $('input[name="schoolName"]', a).val(), c = $('input[name="degree"]', a).val(), d = $('input[name="professionalName"]', a).val(),
                    e = $('input[name="schoolYearStart"]', a).val(), g = $('input[name="schoolYearEnd"]', a).val(), h = $(".eduId", a).val(), i = $("#employee_id").val(), j = $("#resubmitToken").val();
                $(a).find(":submit").val("保存中...").attr("disabled", !0),
                //console.info(h);
                    //todo
                $.updateCnfwsy(h?empResEduUrl + h:empResEduAddUrl, {
                    university_name: b,
                    university_id:h?h:new Date().getTime(),
                    academic_name: c,
                    speciality_name: d,
                    start_year: e,
                    end_year: g,
                    ...
                    //alert(1);
                    $(a).find(":submit").val("保 存").attr("disabled", !1);
                    $("#eduAddCancel").trigger("click");
                    //更新
                    if(h){
                        $(".educationalShow .elist li").each(function(index,ele){
                            if ($(ele).attr("data-id") ==eachItem.university_id ){
                                $(ele).empty();
                                var e = '';
                                e += '<i class="sm_del dn"></i><i class="sm_edit dn"></i><span class="c9" data-startY="' + eachItem.start_year + '" data-endY="' +
                                    ...
                                    + eachItem.academic_name + "，" + eachItem.speciality_name + "</h4>" + "</div>";
                                $(ele).append(e);
                            }
                        });
                        var len = $(".educationalShow .elist li").length;
                        if (len==0){
                        ...
                        var e = '<li data-id="' + eachItem.university_id + '">';
                        e += '<i class="sm_del dn"></i><i class="sm_edit dn"></i><span class="c9" data-startY="' + eachItem.start_year + '" data-endY="' +
                            eachItem.end_year + '">'
                            + eachItem.start_year + "-" + eachItem.end_year + "</span>" + "<div>" + '<h3 data-schName="' + eachItem.university_name + '">'
                            + eachItem.university_name + "</h3>" + '<h4 data-proName="' + eachItem.speciality_name + '" data-degree="' + eachItem.academic_name + '">'
        ...
        obj: $("#selfDescription"), text: $("#selfDescription .selfDescription").val(), Add: function () {
            ...
            this.obj.children(".descriptionEdit").find(".selfDescription").val(""), this.obj.children(".descriptionAdd").addClass("dn"),
                this.obj.children(".descriptionShow").addClass("dn"), this.obj.children(".descriptionEdit").removeClass("dn"), j(this.obj)
        }, Edit: function () {
            tinyMCE.init({
                selector: '#selfDescriptionText',
                menubar: false,
            ...
            b = $.trim(b.replace(/<br>/gi, "")), this.text = b,
                tinyMCE.get('selfDescriptionText').setContent(b),
                ...
                this.obj.children(".descriptionShow").addClass("dn"), this.obj.children(".descriptionEdit").removeClass("dn"),
                this.obj.children(".c_edit").addClass("dn"), j(this.obj)
        }, Cancel: function () {
        ...
        rules: {selfDescription: {required: !1}}, submitHandler: function (a) {
            var b = $('textarea[name="selfDescription"]', a).val(), c = $("#employee_id").val(), d = $("#resubmitToken").val(),e=$("#employee_id").val();
            $(a).find(":submit").val("保存中...").attr("disabled", !0);
            $.updateCnfwsy(empResCustomAddUrl, {description: b, resume_id: c,custom_id: new Date().getTime()}, function (b) {
                console.info(b);
                var c, d;
                b ? (c = b, d = "", c.description ? (d = c.description, g.text = c.description, g.obj.children(".c_edit").removeClass("dn"), g.obj.children(".descriptionShow").html(d).removeClass("dn"), g.obj.children(".descriptionEdit").addClass("dn"),

                    k(g.obj)) : (g.text = "", g.obj.children(".c_edit").addClass("dn"), g.obj.children(".descriptionShow").addClass("dn"), g.obj.children(".descriptionEdit").addClass("dn"),
                    g.obj.children(".descriptionAdd").removeClass("dn"),

                    k(g.obj))) : alert(b.msg), $(a).find(":submit").val("保 存").attr("disabled", !1)

            }, function () {
                alert("fail");
            }, "POST");/*
            $.ajax({
                url: empResCustomAddUrl,
                type: "POST",
                data: {description: b, resume_id: c, employee_id:e},
                async: true,
