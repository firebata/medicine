package com.cnfwsy.spider.email;
...
public class SendMail {
	...
	public static void main(String[] args) throws AddressException {
		send("zzlinks@163.com");
//		byDb();
	}

	private static void byDb() {
		String querySql = "select distinct email from t_ent_company where email is not null ";
		List<Ent_company> companies = DbUtils.queryList(querySql, Ent_company.class);
		boolean flag = false;
		for (Ent_company company : companies) {
			String to = company.getEmail();

			if (to.contains(";")) {
				String[] emails = to.split(";");
				for (String email : emails) {
//					if (email.equals("szjgyy120@163.com")) {
//						flag = true;
//					}
//					if (flag) {
						send(email);
...
//				}
	...
	private static void send(String to) {
		System.out.print("准备发送企业注册邀请函给:" + to + "....");
		// boolean isSSL = true;
		...
		try {
			...
			message.setSubject("［邀请函］国内知名医学生物人才平台招徕网网正在邀请您注册！截止10月30日前注册的企业用户将免费获得半年使用权限！");
